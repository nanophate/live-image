...
import { measureProtectedMaskQuality, measureRgbaLocality } from "../src/motion-comparison.js";
...
test("protected mask quality permits one RGB level in the core and requires clear-mask motion", () => {
  const baseline = rgbaFrame(4, 4);
  const frame = new Uint8ClampedArray(baseline);
  setPixel(frame, 4, 1, 1, [1, 0, 0, 0]);
  setPixel(frame, 4, 2, 1, [0, 9, 0, 0]);
  setPixel(frame, 4, 1, 2, [0, 0, 2, 0]);
  const maskAlpha = new Uint8ClampedArray(16);
  maskAlpha[5] = 255;
  maskAlpha[6] = 0;
  maskAlpha[9] = 255;
  maskAlpha[10] = 0;
  ...
  assert.deepEqual(
    measureProtectedMaskQuality(
      ...
      4,
      4,
      { data: maskAlpha, width: 4, height: 4, originX: 0, originY: 0 },
    ...
    ),
    {
      ...
      coreMaxRgbDelta: 2,
      clearMaskPixels: 2,
      clearMaskChangedPixels: 1,
    },
  );
});

test("protected mask quality ignores feather alpha and can inset region edges", () => {
  const baseline = rgbaFrame(4, 4);
  const frame = new Uint8ClampedArray(baseline);
  setPixel(frame, 4, 0, 0, [10, 0, 0, 0]);
  setPixel(frame, 4, 1, 1, [0, 10, 0, 0]);
  setPixel(frame, 4, 2, 2, [0, 0, 10, 0]);
...
test("protected mask quality consumes a compact Canvas-aligned fractional-region mask", () => {
  const baseline = rgbaFrame(4, 1);
  const frame = new Uint8ClampedArray(baseline);
  setPixel(frame, 4, 0, 0, [2, 0, 0, 0]);
  setPixel(frame, 4, 1, 0, [0, 8, 0, 0]);
  setPixel(frame, 4, 2, 0, [0, 0, 9, 0]);
  ...
  assert.deepEqual(
    measureProtectedMaskQuality(
      ...
      { x: 0.125, y: 0, width: 0.5, height: 1 },
      { regionInsetPixels: 0 },
    ),
    {
      corePixels: 1,
...
test("protected mask quality rejects malformed masks, options, and regions", () => {
  const frame = rgbaFrame(1, 1);
  ...
  assert.throws(
    () => measureProtectedMaskQuality(
      ...
      1,
      1,
      { data: alpha, width: 0, height: 1, originX: 0, originY: 0 },
      region,
    ),
    /width and height/,
  );
  assert.throws(
    () => measureProtectedMaskQuality(
      ...
      1,
      1,
      { data: alpha, width: 1, height: 1, originX: 0.5, originY: 0 },
      region,
    ),
    /origin must contain safe integers/,
  );
  assert.throws(
    () => measureProtectedMaskQuality(
      frame,
      ...
      1,
      1,
      { data: new Uint8ClampedArray(2), width: 1, height: 1, originX: 0, originY: 0 },
      region,
    ),
    /exactly 1 values/,
  );
  assert.throws(
    () => measureProtectedMaskQuality(
      ...
      1,
      1,
      { data: alpha, width: 1, height: 1, originX: 1, originY: 0 },
      region,
      { regionInsetPixels: 0 },
    ),
    /does not cover/,
  ...
  assert.throws(
    () => measureProtectedMaskQuality(frame, frame, 1, 1, { data: alpha, width: 1, height: 1, originX: 0, originY: 0 }, { ...region, x: -1 }),
    /positive normalized rectangle/,
  );
  assert.throws(
    () => measureProtectedMaskQuality(frame, frame, 1, 1, { data: alpha, width: 1, height: 1, originX: 0, originY: 0 }, region, { coreAlphaThreshold: 2, clearAlphaThreshold: 2 }),
    /must be lower/,
  );
  assert.throws(
    () => measureProtectedMaskQuality(frame, frame, 1, 1, { data: alpha, width: 1, height: 1, originX: 0, originY: 0 }, region, { coreRgbTolerance: 1.5 }),
    /8-bit safe integer/,
  );
  assert.throws(
    () => measureProtectedMaskQuality(frame, frame, 1, 1, { data: alpha, width: 1, height: 1, originX: 0, originY: 0 }, region, { regionInsetPixels: -1 }),
