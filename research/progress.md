...
| `3671946` | Cloudflare private-staging security gate | Workers Paid entitlement and disabled route-free deployment, RS256/issuer/AUD/expiry verification for Access assertions with jose 6.2.3, forged-token regressions, private image/version evidence, 55/55 TypeScript, 53/53 Python, both builds, Wrangler dry-run/startup checks, and 14 Chromium passes plus 2 documented local-only skips |
| `52a3208` | Access-before-endpoint operational gate | self-hosted full-Worker Access application, exact-email one-time-PIN Allow policy with 30-minute session, interactive team-domain/AUD Worker secrets, and dashboard confirmation of zero URLs, routes, invocations, errors, and CPU time while the Compiler remains disabled |
| PR #10 | Access-protected staging endpoint | single workers.dev hostname, Preview URLs off, Compiler false, Container rollout none, Worker version `af3a7c73-3ca8-4a78-80d2-9522cc0bb410`, anonymous Viewer/API 302 denial, exact-email OTP success, authenticated hosted-disabled Viewer/config, Allowed audit events, and active/assigned Container instances both zero |
