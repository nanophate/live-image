...
| Cloudflare deployment stack | @cloudflare/containers 0.3.0 ISC; jose 6.2.3 MIT; Wrangler 4.112.0 and Workers types 5.20260719.1 MIT OR Apache-2.0 | no data or sample media | Worker gateway, Access JWT signature/issuer/AUD/expiry validation, static asset upload, and private Container lifecycle | jose and @cloudflare/containers are bundled only into the Worker; deployment tools are not required by the browser runtime; hosted compiler remains disabled by default |
